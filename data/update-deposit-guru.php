...
if ($s1) {
    $up = mysqli_query($koneksi_kantin, "update guru set saldo=saldo+$sel[jumlah] where id=$sel[id_guru]");
    if ($up) {
        $tglNow = date("Y-m-d H:i:s");
        $pesan = "Saldo kamu berhasil di tambahkan sebesar Rp. {$sel['jumlah']} pada $tglNow";
        mysqli_query($koneksi_kantin, "insert into notifikasi(id_guru, pesan, jenis, created_at, updated_at) values ('" . $sel['id_guru'] . "', '$pesan', 'in', current_timestamp(), current_timestamp())");
        die("S");
    } else {
        die("F");
    }
